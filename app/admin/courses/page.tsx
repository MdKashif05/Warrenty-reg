...
export default function AdminCoursesPage() {
  ...
  const handleOpenAdd = () => {
    setForm({ name: "", slug: "", badge: "POPULAR", price: "₹499", lessons: 12, students: 100, desc: "" });
    setEditingSlug(null);
    setShowModal(true);
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (editingSlug) {
      setCourses(courses.map((c: CourseItem) => (c.slug === editingSlug ? { ...form, slug: newSlug } : c)));
    } else {
  ...
  const handleDelete = (slug: string) => {
    if (confirm("Are you sure you want to delete this course?")) {
      setCourses(courses.filter((c: CourseItem) => c.slug !== slug));
    }
  };

  const filtered = courses.filter((c: CourseItem) => c.name.toLowerCase().includes(search.toLowerCase()) || c.badge.toLowerCase().includes(search.toLowerCase()));
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "16px", marginBottom: "28px" }}>
        <div>
          <h1 style={{ fontSize: "28px", fontWeight: "900", color: "#0f172a" }}>
            Thermal Lexum Product & Warranty Management
          </h1>
          <p style={{ fontSize: "14px", color: "#64748b" }}>
            Add, update, or remove thermal paste & cooling product offerings ({courses.length} total items)
          </p>
        </div>
      ...
      <div style={{ marginBottom: "20px" }}>
        <input
          ...
          placeholder="Search items by title or badge..."
              ...
              {filtered.map((course: CourseItem) => (
                <tr key={course.slug} style={{ borderBottom: "1px solid #f1f5f9", fontSize: "14px" }}>
                  <td style={{ padding: "16px 18px" }}>
                    ...
                    <div style={{ fontSize: "12px", color: "#64748b" }}>/products/{course.slug}</div>
                  </td>
                  <td style={{ padding: "16px 18px" }}>
          ...
          <div className="card-nesa" style={{ maxWidth: "560px", width: "100%", padding: "32px", maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
              <h2 style={{ fontSize: "20px", fontWeight: "900", color: "#0f172a" }}>
                {editingSlug ? "Edit Product Item" : "Add New Product Item"}
              </h2>
            ...
            <form onSubmit={handleSave} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              <div>
                <label style={{ fontSize: "12px", fontWeight: "700", color: "#475569", display: "block", marginBottom: "4px" }}>Item Title *</label>
                <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Thermal Lexum TL-X1 Ultra (4g)" style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1px solid #cbd5e1", fontSize: "14px" }} />
              </div>
              ...
              <div className="responsive-grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                <div>
                  ...
                  <input value={form.badge} onChange={(e) => setForm({ ...form, badge: e.target.value })} placeholder="BEST SELLER" style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1px solid #cbd5e1", fontSize: "14px" }} />
                </div>
                <div>
                  <label style={{ fontSize: "12px", fontWeight: "700", color: "#475569", display: "block", marginBottom: "4px" }}>Price *</label>
                  <input required value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} placeholder="₹499" style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1px solid #cbd5e1", fontSize: "14px" }} />
                </div>
              </div>

              <div>
                <label style={{ fontSize: "12px", fontWeight: "700", color: "#475569", display: "block", marginBottom: "4px" }}>Description *</label>
                <textarea rows={3} required value={form.desc} onChange={(e) => setForm({ ...form, desc: e.target.value })} placeholder="Item thermal conductivity details & specifications..." style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1px solid #cbd5e1", fontSize: "14px" }} />
              </div>
              ...
              <div style={{ display: "flex", gap: "12px", justifyContent: "flex-end", marginTop: "12px" }}>
                ...
                <button type="submit" className="btn-primary" style={{ padding: "10px 24px" }}>Save Item 💾</button>
              </div>
            </form>
          </div>
        </div>
